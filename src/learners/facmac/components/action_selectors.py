...
def onehot_from_logits(logits, avail_logits, eps=0.0):
    ...
    agent_outs = ((1 - eps) * agent_outs
                  + th.ones_like(agent_outs) * eps / epsilon_action_num)
    agent_outs[avail_logits == 0] = 0.0
    picked_actions = Categorical(agent_outs).sample()
    picked_actions = th.nn.functional.one_hot(
        picked_actions, num_classes=logits.shape[-1]).float()
...
class GumbelSoftmaxMultinomialActionSelector():
    ...
    def __init__(self, args):
        ...
        self.schedule = DecayThenFlatSchedule(
            args.epsilon_start, args.epsilon_finish,
            args.epsilon_anneal_time, decay="linear")
        self.epsilon = self.schedule.eval(0)
        self.test_greedy = getattr(args, "test_greedy", True)

    def select_action(self, agent_logits, avail_logits,
                      t_env, test_mode=False, explore=False):
        ...
        if test_mode and self.test_greedy:
            ...
            picked_actions = (th.max(
                masked_policies, dim=-1, keepdim=True)[0]
                == masked_policies).float()
        else:
            if not explore:
                picked_actions = GumbelSoftmax(
                    logits=masked_policies).gumbel_softmax_sample()
                picked_actions_hard = \
                    (th.max(picked_actions, dim=-1, keepdim=True)[0]
                     == picked_actions).float()
                picked_actions = \
                    (picked_actions_hard - picked_actions).detach() \
                    + picked_actions
            else:
                # choose between the best and random actions
                # using epsilon greedy
                agent_outs = th.nn.functional.softmax(masked_policies, dim=-1)
                epsilon_action_num = avail_logits.sum(
                ...
                picked_actions = th.nn.functional.one_hot(
                    picked_actions,
                    num_classes=masked_policies.shape[-1]).float()
...
class EpsilonGreedyActionSelector():
    ...
    def __init__(self, args):
        ...
        if getattr(self.args, "epsilon_decay_mode", "decay_then_flat") \
                == "decay_then_flat":
            self.schedule = DecayThenFlatSchedule(
                args.epsilon_start, args.epsilon_finish,
                args.epsilon_anneal_time, decay="linear")
