...
def _post_run_update(prm, record):
    ...
    if prm['RL']['type_learning'] == 'q_learning' \
        ...
        if len(record.q_tables[0][0].keys()) > 0:
            ridx, epoch = 0, 0
            # [state_ind][ridx][epoch][q_table_name][state][action]
            n_states, _ = np.shape(
                record.q_tables[ridx][epoch][prm['RL']['type_Qs'][0]][0])
            for ridx in range(prm['RL']['n_repeats']):
                for epoch in range(prm['RL']['n_epochs']):
                    minq, maxq = _max_min_q(
                        record.q_tables[ridx][epoch],
            ...
            for ridx in range(prm['RL']['n_repeats']):
                minq, maxq = _max_min_q(
                    record.q_tables[ridx], n_states, minq, maxq, prm)
...
def _save_reliability(prm, record):
    ...
    for type_eval in prm['RL']['type_eval']:
        ...
        for ridx in range(prm['RL']['n_repeats']):
            ...
            np.save(save_dir, record.mean_eval_rewards[ridx][type_eval])
