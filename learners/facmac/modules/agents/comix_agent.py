...
class HyperLinear(nn.Module):
    ...
    Linear network layers that allows for two additional complications:
        ...
        - network weights are transformed according to some rule
        before application
    """
    ...
    def forward(self, inputs, weights=None, weight_mod="abs", **kwargs):
        """
        ...
        if self.use_hypernetwork:
            assert weights is not None, \
                "if using hyper-network, need to supply the weights!"
            w = weights
        else:
            w = self.w.weight
...
class CEMAgent(nn.Module):
    def __init__(self, input_shape, rl):
        ...
        self.rl = rl
        num_inputs = input_shape + rl['dim_actions']
        hidden_size = self.rl['rnn_hidden_dim']
    ...
    def forward(self, inputs, hidden_state, actions):
        if actions is not None:
            inputs = th.cat([inputs, actions.contiguous().view(
                -1, actions.shape[-1])], dim=-1)
