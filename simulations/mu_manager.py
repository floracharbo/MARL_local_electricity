...
class Mu_manager:
    """
    Manage computations of the RL flexibilitiy action variable mu.
    ...
    def __init__(self, prm, env):
        """Initialise mu_manager object and add relevant properties."""
        self.name = 'mu manager'
    ...
    def netp_to_mu(self, h, date, netp, loads, home, res):
        """
        ...
        for a in as_:
            if self.aggregate_actions:
                mu_action, bool_flex = self._get_aggregate_mu(
                    mu_action, bool_flex, netp, a
                )
            else:
                mu_action, bool_flex = self._get_disaggregated_mus(
                    mu_action, bool_flex, res, loads, a, h
                )

            error = self._check_mu_errors(
                mu_action, error, res, loads, a, h, bool_flex
            )

        return bool_flex, mu_action, error
    ...
    def initial_processing(self, loads, home):
        ...
        mu['A'], mu['F'] = np.zeros(self.n_agents), np.ones(self.n_agents)
        for i in ['B', 'C', 'D', 'E']:
            ...
            for a in as_:
                ...
                if 1 < mu[i][a] < 1 + 1e-4:
                    mu[i][a] = 1
        self.mu, self.d = mu, d
        self.k, self.x = [[] for _ in range(2)]
    ...
    def apply_step(self, loads, home, mu_action, date, h):
        ...
        for a in as_:
            ...
            if self.aggregate_actions:
                flex_heat = None
                # update variables for given mu_action
                # obtain the interval in which mu lies
                ik = [i for i in range(len(self.x[a]) - 1)
                      if mu_action[a][0] >= self.x[a][i]][-1]
                res = {}  # resulting values (for dp, ds, fl, l)
                for e in self.entries:
                    ik_ = 0 if e == 'dp' else ik
                    # use coefficients to obtain value
                    res[e] = self.k[a][e][ik_][0] * mu_action[a][0] \
                        + self.k[a][e][ik_][1]
                ...
                mu_flex_cons, mu_flex_heat, mu_charge = mu_action[a]
                ...
                # 1 max charge
                res = {}
                mu_flex_cons_ = 0 if mu_flex_cons is None else mu_flex_cons
                loads['flex_cons'].append(mu_flex_cons_ * loads['l_flex'][a])
                flex_heat.append(mu_flex_heat
                                 * self.heat.potential_E_flex()[a])
                home['tot_cons'][a] = self.tot_l_fixed[a] \
                    + loads['flex_cons'][a] \
                    + flex_heat[a]
                res['c'] = home['tot_cons'][a]
                res = self._mu_charge_to_ds(a, mu_charge, res)
        ...
        self.bat.apply_step(self.res)
        self.heat.apply_step(self.res, loads['l_flex'], self.tot_l_fixed,
                             E_flex=flex_heat)
        ...
        bool_penalty = self.bat.check_errors_apply_step(
            as_, bool_penalty, mu_action, self.res)
    ...
    def _mu_charge_to_ds(self, a, mu_charge, res):
        if mu_charge is None:
            ...
            if self.min_charge[a] > 0:
                ...
                res['ds'] = self.min_discharge[a] + abs(mu_charge) \
    ...
    def _plot_graph_mu(self, save_fig=False, name_fig='mu', legend=False):
        line_width = 2.5
        font_size = 15
        ...
        mu = self.mu
        for e in mu.keys():
            ax1.vlines(x=mu[e][0], ymin=y_bottom, ymax=y_top,
                       color='gray', linestyle='--', linewidth=0.5)
        ax1.hlines(y=0, xmin=x_left, xmax=x_right,
    ...
    def _compute_k(self, a, a_dp, b_dp, mu, d):
        letters = ['A', 'B', 'C', 'D', 'E', 'F']
                ...
                for e in ['ds', 'c', 'losses']:
                    if e == 'losses':
                        self.k = self.bat.k_losses(
                            a, self.k, mu[l1][a], mu[l2][a])
                    else:
                        ad = (d[e][l2][a] - d[e][l1][a]) / \
                             (mu[l2][a] - mu[l1][a])
                        bd = d[e][l2][a] - mu[l2][a] * ad
    ...
    def _get_aggregate_mu(self, mu_action, bool_flex, netp, a):
        if abs(self.k[a]['dp'][0][0]) < 1e-2:
            self.k[a]['dp'][0][0] = 0
        if self.k[a]['dp'][0][0] == 0:  # there is not flexibility
            # boolean for whether or not we have flexibility
            bool_flex.append(0)
            mu_action.append([None])
        else:
            bool_flex.append(1)
            # mu none if no flexibility
            ...
            mu_action.append([None if self.k[a]['dp'][0][0] == 0
                              else delta / self.k[a]['dp'][0][0]])
        if mu_action[a] is not None:
            assert - 1e-2 < mu_action[a][0] < 1 + 1e-2, \
                "mu_action should be between 0 and 1"

        return mu_action, bool_flex

    def _get_disaggregated_mus(self, mu_action, bool_flex, res, loads, a, h):
        ...
        if (
            ...
            mu_charge = (res['charge'][a, h] - min_charge_a) \
                / (max_charge_a - min_charge_a)
        mu_action.append([mu_flex_cons, mu_flex_heat, mu_charge])
        bool_flex.append(
            False if sum(mu is None for mu in mu_action[a]) == 3
            else True)

        return mu_action, bool_flex

    def _check_mu_errors(self, mu_action, error, res, loads, a, h, bool_flex):
        for i in range(self.dim_actions):
            ...
            if error[a]:
                print(f"h {h} mu_action[{a}] = {mu_action[a]}")
                np.save('res_error', res)
                np.save('loads', loads)
                np.save('E_heat_min', self.heat.E_heat_min)
                np.save('E_heat_max', self.heat.E_heat_max)
                np.save('mu_action_error', mu_action)

        mu_none = (self.aggregate_actions and mu_action[a] is None) \
            or (not self.aggregate_actions
                and all(mu is None for mu in mu_action[a]))
        assert not (bool_flex[a] is True and mu_none), \
            f"mu[{a}] is none whereas there is flexibility"

        return error

# figs1
# env.mu_manager.mincharge=[7.5,7.5]
# env.mu_manager.maxcharge=[75,75]
# env.mu_manager.initial_processing(
# [45,45], [10, 10], [10,10], [1,1], [5,5],[20, 20],[5, 5],[0, 1])
# env.mu_manager._plot_graph_mu(True, 'mu_smallgen')

# env.mu_manager.mincharge=[7.5,7.5]
# env.mu_manager.maxcharge=[30,30]
# env.mu_manager.store0=[37.5,37.5]
# env.mu_manager.initial_processing(
# [20,20], [10, 10], [15,15], [1,1], [0,0],[10, 10],[0, 0],[0, 1])
# env.mu_manager._plot_graph_mu(True, 'mu_midgen')
...
# env.mu_manager._plot_graph_mu(True, 'mu_largegen')
...
# env.mu_manager.initial_processing(
# [35,35], [20, 20], [5,5], [1,1], [0,0],[20, 20],[0, 0],[0, 1])
# env.mu_manager._plot_graph_mu(True, 'mu_legend', legend=True)
